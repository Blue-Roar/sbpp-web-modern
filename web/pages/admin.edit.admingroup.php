...
if (isset($_POST['wg']) || isset($_GET['wg']) || isset($_GET['sg'])) {
    ...
    if ($_POST['wg'] > 0 && (empty($password) || empty($email))) {
        echo '<script>ShowBox("Error", "Admins have to have a password and email set in order to get web permissions.<br /><a href=\"/admin?c=admins&o=editdetails&id=' . $_GET['id'] . '\" title=\"Edit Admin Details\">Set the details</a> first and try again.", "red");</script>';
    } else {
        ...
        if (Config::getBool('config.enableadminrehashing')) {
            ...
            echo '<script>ShowRehashBox("' . implode(",", $allservers) . '", "Admin updated", "The admin has been updated successfully", "green", "/admin?c=admins");TabToReload();</script>';
        } else {
            echo '<script>ShowBox("Admin updated", "The admin has been updated successfully", "green", "/admin?c=admins");TabToReload();</script>';
        }
