    ...
    <div class="tabcontent" id="List servers">
        ...
        {else}
                    ...
                    <tr id="sid_{$server.sid}" {if $server.enabled==0}style="background-color:#eaeaea" title="Disabled"{/if}>
                        ...
                        <td style="border-bottom: solid 1px #ccc" height='16'>
                            {if $server.rcon_access}
                                <a href="/admin?c=servers&o=rcon&id={$server.sid}">RCON</a> -
                            {/if}

                            <a href="/admin?c=servers&o=admincheck&id={$server.sid}">Admins</a>

                            {if $permission_editserver}
                                - <a href="/admin?c=servers&o=edit&id={$server.sid}">Edit</a>
                            {/if}
            ...
            {if $permission_addserver}
                <br />
                <div class="rowdesc">
                    <span class="text-info cursor-help me-1" data-bs-toggle="tooltip" data-bs-placement="right" title="Click here to upload a screenshot of a map. Use the mapname as filename."><i class="fas fa-circle-question icon"></i></span> Upload Map Image
